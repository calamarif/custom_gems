...
from prophecy.utils import get_alias, add_rule
...
class SchemaTransform(ComponentSpec):
    ...
    docUrl: str = "https://docs.prophecy.io/engineers/schema-transform/"
                ...
                if isinstance(tf, SchemaTransform.AddReplaceColumn):
                    ...
                    if tf.sourceColumn.value is not None:
                        if(len(tf.sourceColumn.value) == 0):
                            ...
                            output_columns_processed.add(tf.sourceColumn.value)
                    else:
                        pass
                ...
                if isinstance(tf, self.RenameColumn):
                    ...
                    if tf.sourceColumn.value is not None and tf.targetColumn.value is not None:
                        if (len(tf.sourceColumn.value.strip()) == 0 or len(tf.targetColumn.value.strip()) == 0):
                            ...
                            output_columns_processed.add(tf.targetColumn.value)
                    else:
                        pass
                    ...
                    if len(tf.expression.rawExpression.strip()) == 0:
                        ...
                        for rule in rules:
                            if rule.matches_expression(tf.expression.rawExpression):
                                rule_outputs = rule.outputs if len(rule.outputs) > 0 else None
                                ...
                                continue
                        if rule_outputs:
                            output_names = [out.name for out in rule_outputs]
                            transformed_columns.extend(output_names)
                            duplicate_outputs = output_columns_processed.intersection(set(output_names))
                            if duplicate_outputs:
                                diagnostics.append(Diagnostic(
                                    f"properties.transformations[{idx}].expression.expression",
                                    f"Duplicate outputs found: {', '.join(duplicate_outputs)}",
                                    SeverityLevelEnum.Warning
                                ))
                            output_columns_processed.update(output_names)
                        else:
            ...
            try:
                for (idx, line) in enumerate(parse_escaped_csv(component.properties.importString, field_min=2, field_max=3)):
                    ...
                    if tf_type not in allowed_tfs:
                        ...
                        all_rules: List[BusinessRule] = context.functions_context.rules
                        rule_matches = False
                        for rule in all_rules:
                            if rule.matches_expression(line[1].strip()):
                                rule_matches = True
                                break
                        if not rule_matches:
                            msg = f"Rule with the name: `{line[1].strip()}` is not defined or broken. Please verify."
        ...
        for transformation in newProps.transformations:
            ...
            if isinstance(transformation, self.AddReplaceColumn):
                ...
                if rule_outputs is not None:
                    output_names = [out.name for out in rule_outputs]
                    duplicate_columns = set(transformed_columns).intersection(set(output_names))
                    if duplicate_columns:
                        for duplicate_column in duplicate_columns:
                            all_param_errors.append(ParamError(paramName=duplicate_column, hasError=True))
                    transformed_columns.extend(output_names)
        ...
        def apply(self, spark: SparkSession, in0: DataFrame) -> DataFrame:
            ...
            for transformation in self.props.transformations:
                if isinstance(transformation, SchemaTransform().AddReplaceColumn):
                    ...
                    out = add_rule(out, transformation.expression.column())
            return out