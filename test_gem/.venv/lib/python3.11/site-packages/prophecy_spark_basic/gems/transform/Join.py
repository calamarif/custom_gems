...
class Join(ComponentSpec):
    ...
    docUrl: str = "https://docs.prophecy.io/engineers/join/"
    ...
    def allColumnsSelectionFunc(self, portId: str, state: Component[JoinProperties]):
        if state.properties.activeTab == "expressions":
            ...
            return state.bindProperties(dataclasses.replace(state.properties, expressions=updatedExpressions))
        else:
            return state
        ...
        return Dialog("Join").addElement(
            ColumnsLayout(gap=("1rem"), height=("100%"))
                ...
                Ports(
                    ...
                    allColumnsSelectionCallback=self.allColumnsSelectionFunc).editableInput(True), "content"
            )
                .addColumn(
                Tabs()
                    ...
                    TabPane("Conditions", "conditions")
                        ...
                        StackLayout(height=("100%"))
                            ...
                            "Test",
                            columns=[
                                Column("Input Alias", "alias", width="20%"),
                                Column(
                                    "Join Condition",
                        ...
                        BasicTable(
                            ...
                            columns=[
                                ...
                                Column("Propagate All Columns", "propagateColumns",
                                       (Checkbox("").bindProperty("record.propagateColumns")), width="20%")
                            ],
                            delete=False,
                            appendNewRow=False,
                            targetColumnKey="alias",
    ...
    def validate(self, context: WorkflowContext, component: Component[JoinProperties]) -> List[Diagnostic]:
        ...
        if (len(component.ports.inputs) < 2):
            ...
            # Sending None as testColumnPresence to avoid column presence validation to support lateral column aliases.
            ...
            d2 = [x.appendMessage("[Expressions]").setDiagnosticFix(CopilotDiagnosticFix("expression")) for x in _d2]
            diagnostics.extend(d2)

        return diagnostics

    def getCPStmt(self, component: Component[JoinProperties]) -> Optional[SelectStmt]:
        projectionExpressions = [ProjectionExpression(expression.target, expression.expression.rawExpression) for
                                 expression in
                                 component.properties.expressions]
        joinExpressions = [JoinExpression(expression.alias, expression.joinType, expression.expression.rawExpression)
                           for expression in component.properties.conditions]
        return SelectStmt(
            gemType=component.component,
    ...
    def onChange(self, context: WorkflowContext, oldState: Component[JoinProperties], newState: Component[JoinProperties]) -> Component[
        ...
        conditionsAfterChangeApplication: list[JoinCondition] = [dataclasses.replace(x) for x in oldConditions]
        ...
        for (port, idx) in slugChanged:
            if idx - 1 >= 0:
                ...
                conditionsAfterChangeApplication[idx - 1] = dataclasses.replace(conditionsAfterChangeApplication[idx - 1],
                                                                    alias=newAlias)
