    ...
    def dialog(self) -> Dialog:
        ...
        runDDLDialog = (
            Condition()
                ...
                .then(
                TextArea("",6,placeholder="use {table_name} as a placeholder for the name of the table or mention the complete table name."+"\n\n"+
                                          "Example 1: alter {table_name} ADD COLUMNS (col1 STRING COMMENT 'New column for notes',col2 TIMESTAMP)"+"\n"+
                                          "Example 2: alter test_catalog.test_schema.test_table ADD COLUMNS (col1 STRING COMMENT 'New column for notes',col2 TIMESTAMP)"
                         ).bindProperty("runDDL")
            )
        )
            ...
            if not ("SColumnExpression" in locals()):
                ...
                if self.props.action == "registerTableInCatalog":
                    ...
                    if (
                        ...
                        spark.sql(
                            f"OPTIMIZE " + table_name + f" WHERE {self.props.optimiseWhere}"
                        )
                    elif not self.props.useOptimiseWhere:
                        spark.sql(
                            f"OPTIMIZE " + table_name + f" ZORDER BY {zorderstr}"
                        )
                    else:
                        spark.sql(
                            f"OPTIMIZE " + table_name + f" WHERE {self.props.optimiseWhere} ZORDER BY {zorderstr}"
                        )
