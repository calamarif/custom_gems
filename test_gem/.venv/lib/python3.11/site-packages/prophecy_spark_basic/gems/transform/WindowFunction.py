...
class WindowFunction(ComponentSpec):
    ...
    gemDescription: str = (
        "Define a WindowSpec and apply Window functions on a DataFrame."
    )
    docUrl: str = "https://docs.prophecy.io/engineers/window-function/"
    ...
    def onClickFunc(
        self, portId: str, column: str, state: Component[WindowFunctionProperties]
    ):
        if state.properties.activeTab == "windowPartition":
            ...
            return state.bindProperties(
                dataclasses.replace(state.properties, partitionColumns=partitionColumns)
            )
        elif state.properties.activeTab == "windowOrder":
            orderColumns = state.properties.orderColumns
            orderColumns.append(
                OrderByRule(SColumn.getSColumn(sanitizedColumn(column)), "asc")
            )
            return state.bindProperties(
                dataclasses.replace(state.properties, orderColumns=orderColumns)
            )
            ...
            return state.bindProperties(
                dataclasses.replace(
                    state.properties, expressionColumns=expressionColumns
                )
            )
        else:
            return state
    ...
    def allColumnsSelectionFunc(
        self, portId: str, state: Component[WindowFunctionProperties]
    ):
        columnsInSchema = getColumnsInSchema(portId, state, SchemaFields.TopLevel)
        if state.properties.activeTab == "windowPartition":
            partitionColsToBeAdded = list(
                map(
                    lambda column: SColumn.getSColumn(sanitizedColumn(column)),
                    columnsInSchema,
                )
            )
            ...
            return state.bindProperties(
                dataclasses.replace(
                    state.properties, partitionColumns=updatedPartitionColumns
                )
            )
        elif state.properties.activeTab == "windowOrder":
            orderColsToBeAdded = list(
                map(
                    lambda column: OrderByRule(
                        SColumn.getSColumn(sanitizedColumn(column)), "asc"
                    ),
                    columnsInSchema,
                )
            )
            ...
            return state.bindProperties(
                dataclasses.replace(state.properties, orderColumns=updatedOrderCols)
            )
        elif state.properties.activeTab == "windowUse":
            expressionsToBeAdded = list(
                map(
                    lambda column: SColumnExpression(column, SColumn("row_number()")),
                    columnsInSchema,
                )
            )
            ...
            return state.bindProperties(
                dataclasses.replace(
                    state.properties, expressionColumns=updatedExpressions
                )
            )
        else:
            return state
    ...
    def dialog(self) -> Dialog:
        ...
        rowStartColumn = (
            ...
            .addElement(
                Condition()
                .ifEqual(
                    PropExpr("component.properties.frameStart"),
                    StringExpr("userPreceding"),
                )
                .then(
                    TextBox("Number of rows")
                    .bindPlaceholder("-1")
                    .bindProperty("userSpecifiedStart")
                )
            )
        )
        rowEndColumn = (
            ...
            .addElement(
                Condition()
                .ifEqual(
                    PropExpr("component.properties.frameEnd"),
                    StringExpr("userFollowing"),
                )
                .then(
                    TextBox("Number of rows")
                    .bindPlaceholder("1")
                    .bindProperty("userSpecifiedEnd")
                )
            )
        )
        ...
        windowPane1 = TabPane("PartitionBy", "windowPartition").addElement(
            partitionTable
        )
        windowPane2 = TabPane("OrderBy", "windowOrder").addElement(orderTable)
        windowPane3 = TabPane("Frame", "windowFrame").addElement(
            StackLayout()
            .addElement(Checkbox("Specify Frame").bindProperty("specifyFrame"))
            .addElement(
                NativeText("Unselecting above uses default frame specifications.")
            )
            .addElement(
                Condition()
                .ifEqual(
                    PropExpr("component.properties.specifyFrame"), BooleanExpr(True)
                )
                .then(
                    StackLayout(height="100%")
                    .addElement(
                    ...
                    .addElement(
                        Condition()
                        .ifEqual(
                            PropExpr("component.properties.frameType"),
                            StringExpr("row"),
                        )
                        .then(
        ...
        windowPane4 = TabPane("Window Use", "windowUse").addElement(
            StackLayout(height="100%")
            .addElement(
                NativeText(
                    "The below expressions are already computed over the defined window "
        ...
        return Dialog("WindowFunction").addElement(
            ColumnsLayout(gap="1rem", height="100%")
            .addColumn(portSchemaTabs, "content")
            .addColumn(windowTabs, "5fr")
        )

    def validate(
        self, context: WorkflowContext, component: Component[WindowFunctionProperties]
    ) -> List[Diagnostic]:
        diagnostics = []
        ...
        if len(component.properties.expressionColumns) == 0:
            diagnostics.append(
                Diagnostic(
                    "properties.expressionColumns",
                    "At least one expression in required. [Window Use]",
                    SeverityLevelEnum.Error,
                )
            )
        else:
            expressionDiagnostics = validateExpTable(
                component.properties.expressionColumns, "expressionColumns", component
            )
            diagnostics.extend(
                [x.appendMessage("[Window Use]") for x in expressionDiagnostics]
            )
        ...
        if len(component.properties.orderColumns) > 0:
            for idx, expr in enumerate(component.properties.orderColumns):
                ...
                diagnostics.extend(
                    [x.appendMessage("[OrderBy]") for x in orderColDiagnoistics]
                )
        ...
        if len(component.properties.partitionColumns) > 0:
            for idx, expr in enumerate(component.properties.partitionColumns):
                ...
                diagnostics.extend(
                    [x.appendMessage("[PartitionBy]") for x in partitionColDiagnoistics]
                )

        if component.properties.specifyFrame:
            if (
                component.properties.frameType == "range"
                and len(component.properties.orderColumns) == 0
            ):
                diagnostics.append(
                    Diagnostic(
                ...
                if not isBlank(component.properties.frameStart) and not isBlank(
                    component.properties.frameEnd
                ):
                    if component.properties.frameStart == "currentRow":
                        ...
                        if isBlank(component.properties.userSpecifiedStart):
                            ...
                            frameStartInt = parseInt(
                                component.properties.userSpecifiedStart
                            )
                    else:
                        frameStartInt = 0
                    ...
                    if component.properties.frameEnd == "currentRow":
                        ...
                        if isBlank(component.properties.userSpecifiedEnd):
                            ...
                            frameEndInt = parseInt(
                                component.properties.userSpecifiedEnd
                            )
                    else:
                        frameEndInt = 0
                    if (
                        frameStartInt is not None
                        and frameEndInt is not None
                        and frameStartInt > frameEndInt
                    ):
                        diagnostics.append(
                            Diagnostic(
                                "properties.userSpecifiedStart",
    ...
    def onChange(
        ...
        oldState: Component[WindowFunctionProperties],
        newState: Component[WindowFunctionProperties],
    ) -> Component[WindowFunctionProperties]:
        newProps = newState.properties
        usedCols = getColumnsToHighlight(
            newProps.partitionColumns
            + newProps.expressionColumns
            + [x.expression for x in newProps.orderColumns],
            newState,
        )
        updatedprops = newProps
        if newProps.frameType != oldState.properties.frameType:
            updatedprops = dataclasses.replace(
                updatedprops, frameStart=None, frameEnd=None
            )
        if newProps.frameStart != oldState.properties.frameStart:
            updatedprops = dataclasses.replace(updatedprops, userSpecifiedStart=None)
        if newProps.frameEnd != oldState.properties.frameEnd:
            updatedprops = dataclasses.replace(updatedprops, userSpecifiedEnd=None)

        return newState.bindProperties(
            dataclasses.replace(
                updatedprops, columnsSelector=usedCols, activeTab=newProps.activeTab
            )
        )
    ...
    class WindowFunctionCode(ComponentCode):
        ...
        def apply(self, spark: SparkSession, in0: DataFrame) -> DataFrame:
            orderRules = map(
                ...
                if (x.sortType == "asc")
                else x.expression.column().asc_nulls_last()
                if (x.sortType == "asc_nulls_last")
                else x.expression.column().desc_nulls_first()
                if (x.sortType == "desc_nulls_first")
            ...
            if len(self.props.partitionColumns) > 0:
                partSpec: WindowSpec = Window.partitionBy(
                    *list(map(lambda x: x.column(), self.props.partitionColumns))
                )
            else:
                partSpec: WindowSpec = Window.partitionBy()
            ...
            for expression in self.props.expressionColumns:
                out = out.withColumn(
                    expression.target,
                    expression.expression.column().over(windowWithFrame),
                )
