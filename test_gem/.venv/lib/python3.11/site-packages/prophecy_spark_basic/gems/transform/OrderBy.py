...
class OrderBy(ComponentSpec):
    ...
    docUrl: str = "https://docs.prophecy.io/engineers/order-by/"
    ...
    def onClickFunc(self, portId: str, column: str, state: Component[OrderByProperties]):
        ...
        return state.bindProperties(dataclasses.replace(state.properties, orders=orders))
    ...
    def allColumnsSelectionFunc(self, portId: str, state: Component[OrderByProperties]):
        ...
        return state.bindProperties(dataclasses.replace(state.properties, orders=updatedOrders))
            ...
            ColumnsLayout(gap=("1rem"), height=("100%"))
                ...
                BasicTable("OrderByTable", columns=[Column(
                    ...
                    Column("Sort", "sortType", SelectBox("")
                           ...
                           .addOption("Ascending Nulls Last", "asc_nulls_last")
                           .addOption("Descending Nulls First", "desc_nulls_first")
                           .addOption("Descending Nulls Last", "desc"), width="25%")
                ])
                    .bindProperty("orders"),
                "5fr"
            )
    ...
    def onChange(self, context: WorkflowContext, oldState: Component[OrderByProperties], newState: Component[OrderByProperties]) -> \
        ...
        orders = [x if not x.sortType == "" else OrderByRule(SColumn.getSColumn(x.expression.rawExpression), "asc") for
                  x in orders]
        return newState.bindProperties(dataclasses.replace(newProps, columnsSelector=usedColExps, orders=orders))
