...
class Deduplicate(ComponentSpec):
    ...
    docUrl: str = "https://docs.prophecy.io/engineers/deduplicate/"
    ...
    def onClickFunc(
        ...
        return state.bindProperties(
            dataclasses.replace(state.properties, dedupColumns=dedups)
        )
    ...
    def allColumnsSelectionFunc(
        ...
        return state.bindProperties(
            dataclasses.replace(state.properties, dedupColumns=updatedDedups)
        )
    ...
    def validate(
        self, context: WorkflowContext, component: Component[DeduplicateProperties]
    ) -> List[Diagnostic]:
        diagnostics = []
    ...
    class DeduplicateCode(ComponentCode):
        ...
        def apply(self, spark: SparkSession, in0: DataFrame) -> DataFrame:
            ...
            window = Window.partitionBy(*groupByColumns).orderBy(*orderRules)
            ...
            if typeToKeep == "any":
                ...
                return (
                    dataFrameWithRowNumber.filter(col("row_number") == col("count"))
                    .drop("row_number")
                    .drop("count")
                )
