...
class csv(DatasetSpec):
    ...
    docUrl: str = "https://docs.prophecy.io/engineers/csv/"
    ...
    def sourceDialog(self) -> DatasetDialog:
        ...
        pandasSection = (ColumnsLayout(gap=("1rem"), height=("100%"))
                        ...
                        ScrollBox().addElement(
                            StackLayout(height=("100%"))
                                .addElement(Checkbox("Enforce schema").bindProperty("pandasUseSchema"))
            ...
            "PROPERTIES",
                StackLayout(height=("100%"))
                    .addElement(
                        Condition()
                            .ifNotEqual(
                            PropExpr("component.properties.pathSelection"),
                            StringExpr("fileLocation"),
                        ).then(
                            SelectBox("Read With")
                                .addOption("Spark", "spark")
                                .addOption("Pandas", "pandas")
                                .bindProperty("readWith")
                        )
                    )
                    .addElement(
                        Condition()
                            .ifEqual(
                            PropExpr("component.properties.readWith"),
                            StringExpr("pandas"),
                        )
                        .then(
                            AlertBox(
                                ...
                                _children=[
                                    Markdown(
                                        "Use this to read csv files from **SFTP/Sharepoint**. Requirements: configure the [pandas](https://pypi.org/project/pandas/) PyPi dependency on the Spark cluster"
                                    )
                                ],
                            )
                        )
    ...
    def validate(self, context: WorkflowContext, component: Component) -> list:
        ...
        if component.properties.targetPathSelection not in ["sharepoint", "sftp"] and (component.properties.pathSelection in ["sharepoint", "fileLocation"]) and len(component.properties.path) == 0:
            if component.properties.pathSelection == "sharepoint":
                diagnostics.append(
                    Diagnostic("properties.path", "File location cannot be empty", SeverityLevelEnum.Error)
                    )
            if component.properties.pathSelection == "fileLocation":
                diagnostics.append(
                    Diagnostic("properties.path", "File location cannot be empty", SeverityLevelEnum.Error)
                    )
        ...
        if component.properties.pathSelection == "sharepoint":
            if component.properties.credType == "userPwd":
                if not component.properties.secretUsername.parts:
                    diagnostics.append(Diagnostic("properties.secretUsername", "Username cannot be empty",
                                                SeverityLevelEnum.Error))
                if not component.properties.secretPassword.parts:
                    diagnostics.append(Diagnostic("properties.secretPassword", "Password cannot be empty",
                                                SeverityLevelEnum.Error))

            if component.properties.credType == "clientSecrets":
                if not component.properties.tenantId.parts:
                    diagnostics.append(Diagnostic("properties.tenantId", "Tenant ID cannot be empty",
                                                SeverityLevelEnum.Error))
                if not component.properties.clientId.parts:
                    diagnostics.append(Diagnostic("properties.clientId", "Client ID cannot be empty",
                                                SeverityLevelEnum.Error))
                if not component.properties.clientSecretId.parts:
                    diagnostics.append(Diagnostic("properties.clientSecretId", "Client Secret ID cannot be empty",
                                                SeverityLevelEnum.Error))

            if not component.properties.secretUrl.parts:
                diagnostics.append(
                    Diagnostic("properties.secretUrl", f"Sharepoint URL cannot be empty",
                               SeverityLevelEnum.Error))

        if component.properties.pathSelection == "sftp":
            if not component.properties.sftpSecretUsername.parts:
                diagnostics.append(Diagnostic("properties.sftpSecretUsername", "Username cannot be empty",
                                              SeverityLevelEnum.Error))
            if not component.properties.sftpSecretPassword.parts:
                diagnostics.append(Diagnostic("properties.sftpSecretPassword", "Password cannot be empty",
                                              SeverityLevelEnum.Error))

            if not component.properties.sftpSecretUrl.parts:
                diagnostics.append(
                    Diagnostic("properties.sftpSecretUrl", f"Host cannot be empty",
                               SeverityLevelEnum.Error))

            if not component.properties.sftpSourcePath:
                diagnostics.append(
                    Diagnostic("properties.sftpSourcePath", f"File location cannot be empty",
                               SeverityLevelEnum.Error))
    ...
    def onChange(self, context: WorkflowContext, oldState: Component, newState: Component) -> Component:
        ...
        if newState.properties.schema is not None:
            ...
            if oldCR is not None and newCR is None:
                ...
                schema = newSchema
            updatedprops = dataclasses.replace(updatedprops, schema=schema)
        
        if newState.properties.pathSelection == "fileLocation":
            updatedprops = dataclasses.replace(updatedprops, readWith="spark")

        return newState.bindProperties(dataclasses.replace(updatedprops))
        ...
        def sourceApply(self, spark: SparkSession) -> DataFrame:
            ...
            if self.props.pathSelection == "sharepoint":
                if self.props.credType == "userPwd":
                    ...
                    file_item: SubstituteDisabled = client.sites[site_id].drive.root.get_by_path(file_url).get().execute_query()
                    download_path: SubstituteDisabled = os.path.join("/tmp", os.path.basename(file_url))

                    with open(download_path, "wb") as local_file:
                        file_item.download(local_file).execute_query()

                if self.props.readWith == "spark":
                    with open(download_path, 'r') as f:
                        lines: SubstituteDisabled = f.readlines()

                    rdd: SubstituteDisabled = spark.sparkContext.parallelize(lines)
                    finalPath = rdd
                elif self.props.readWith == "pandas":
                    import pandas as pd               
                    ...
                    if self.props.separator is not None and self.props.separator != "":
                        kwargs["sep"] = self.props.separator
                    if self.props.encoding is not None and self.props.encoding != "":
                        kwargs["encoding"] = self.props.encoding
                    if self.props.escape is not None and self.props.escape != "":
                        kwargs["escapechar"] = self.props.escape
                    if self.props.quote is not None and self.props.quote != "":
                        kwargs["quotechar"] = self.props.quote
                    ...
                    pandasDf: SubstituteDisabled = pd.read_csv(download_path, **kwargs)
                    if self.props.pandasUseSchema and self.props.pandasUseSchema is not None:
            ...
            if self.props.pathSelection == "fileLocation" or self.props.readWith == "spark":
                if self.props.skipHeaders or self.props.skipFooters:
                    skipHeaderLines = (int(self.props.skipHeaders.strip()) if self.props.skipHeaders and self.props.skipHeaders.strip().isdigit() else 0)
                    if self.props.encoding is not None:
                        linesRDD = spark.sparkContext.binaryFiles(self.props.path).flatMap(lambda x: x[1].decode(self.props.encoding).splitlines())
                    else:
                        linesRDD = spark.sparkContext.binaryFiles(self.props.path).flatMap(lambda x: x[1].decode("utf-8").splitlines())
                    totalCount = linesRDD.count()
                    skipFooterLines = 0
                    if(totalCount>0):
                        skipFooterLines = (totalCount - (int(self.props.skipFooters.strip()) if self.props.skipFooters and self.props.skipFooters.strip().isdigit() else 0))
                    finalRDD = linesRDD.zipWithIndex().filter(lambda x: skipHeaderLines <= x[1] < skipFooterLines).map(lambda x: x[0])
                    finalPath = finalRDD
                reader = spark.read
                ...
                if (self.props.encoding is not None) and (self.props.skipHeaders is None) and (self.props.skipFooters is None):
