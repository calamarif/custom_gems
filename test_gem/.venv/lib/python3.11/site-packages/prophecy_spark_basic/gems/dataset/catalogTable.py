...
class catalogTable(DatasetSpec):
    ...
    docUrl: str = "https://docs.prophecy.io/engineers/delta-table"
    ...
    class CatalogTableProperties(DatasetProperties):
        ...
        mergeSchemaEvolution: Optional[bool] = None
        updRemainingColsFromSource: Optional[bool] = False
    ...
    def targetDialog(self) -> DatasetDialog:
                                ...
                                .then(
                                    ...
                                    .addColumn(
                                        ...
                                        .then(
                                            ...
                                            .addElement(Checkbox("Update Remaining Columns from Source").bindProperty("updRemainingColsFromSource"))
                                        )
                                    )
                                )
                            )
        ...
        scd2View = StackLayout() \
            ...
            .addElement(TitleElement("Soft Delete")) \
            .addElement(
            Checkbox("Enable Soft Delete").bindProperty("enableSoftDelete")
        )
        ...
        deltaFieldPicker = fieldPicker.addField(
            ...
            .addField(Checkbox("Schema evolution for merge and SCD2 wtite mode"), "mergeSchemaEvolution") \
            .addField(
    ...
    class CatalogTableCode(ComponentCode):
        ...
        def targetApply(self, spark: SparkSession, in0: DataFrame):
            ...
            if self.props.writeMode == "merge" and table_exists:
                ...
                if self.props.mergeSchemaEvolution:
                    dt = dt.withSchemaEvolution()
                ...
                if self.props.matchedAction == "update":
                    ...
                    if len(self.props.matchedTable) > 0:
                        ...
                        if self.props.updRemainingColsFromSource:
                            col_list: SubstituteDisabled = in0.columns
                            merge_source_alias = self.props.mergeSourceAlias
                            for column_name in col_list:
                                if column_name not in matched_expr:
                                    matched_expr[column_name] = f"{merge_source_alias}.{column_name}"
                ...
                updatesDF: SubstituteDisabled = in0.withColumn(minFlagColumn, lit(flagY)).withColumn(maxFlagColumn,
                                                                                                     lit(flagY)) if (self.props.createMinMaxFlags) else in0
                ...
                if self.props.createMinMaxFlags:
                    ...
                    if self.props.enableSoftDelete:
                        # ── duplicate only rows whose toTime IS NULL (still active) ──
                        insertCopyDF = (
                            rowsToUpdate
                            .filter(col(toTimeColumn).isNull())  # skip soft deletes
                            .withColumn("mergeKey", lit(None))  # forces INSERT
                        )
                        updateCopyDF = updatesDF.withColumn("mergeKey", concat(*keyColumns))
                        stagedUpdatesDF = insertCopyDF.unionByName(updateCopyDF)
                    else:
                    ...
                    if self.props.mergeSchemaEvolution:
                        mergeTable = mergeTable.withSchemaEvolution()

                    if self.props.enableSoftDelete:
                        mergeTable \
                            .whenMatchedUpdate(
                            # ① SOFT DELETE  (incoming toTime IS NOT NULL)
                            condition=(
                                    (col(f"existingDF.{maxFlagColumn}") == lit(flagY)) &  # current row
                                    col(f"staged_updates.{toTimeColumn}").isNotNull()  # soft-delete row
                            ),
                            set={
                                toTimeColumn: f"staged_updates.{toTimeColumn}"
                            }
                        ) \
                            .whenMatchedUpdate(
                            # ② REGULAR CHANGE  (toTime IS NULL & attrs differ)
                            condition=(
                                    (col(f"existingDF.{maxFlagColumn}") == lit(flagY))  # current row (unchanged)
                                    & col(f"staged_updates.{toTimeColumn}").isNull()  # ← new guard we added
                                    & updateCond  # historic diff (unchanged)
                            ),
                            set={
                                maxFlagColumn: flagN,
                    ...
                    rowsToUpdate = updatesDF.alias("updatesDF") \
                        .join(existingDF.alias("existingDF"), keyColumns) \
                        .filter(
                        (col("existingDF." + toTimeColumn).isNull() & (~col("existingDF." + fromTimeColumn).isNull())) & (
                            cond
                        )) \
                        .select(*[col("updatesDF." + val) for val in updateColumns])
                    ...
                    mergeTableMinMax = existingTable \
                        .alias("existingDF") \
                        .merge(
                        stagedUpdatesDF.alias("staged_updates"),
                        concat(*[col("existingDF." + key) for key in keyColumns]) == col("staged_updates." + "mergeKey")
                    ...
                    if self.props.mergeSchemaEvolution:
                        mergeTableMinMax = mergeTableMinMax.withSchemaEvolution()

                    if self.props.enableSoftDelete:
                        mergeTableMinMax \
                            .whenMatchedUpdate(
                            # ① SOFT-DELETE: incoming row already has toTime
                            condition=(
                                    col(f"existingDF.{toTimeColumn}").isNull() &  # current row
                                    col(f"staged_updates.{toTimeColumn}").isNotNull()  # soft delete
                            ),
                            set={
                                toTimeColumn: f"staged_updates.{toTimeColumn}"  # set end
                            }
                        ) \
                            .whenMatchedUpdate(
                            # ② REGULAR SCD-2: attributes changed, still active
                            condition=(
                                    col(f"existingDF.{toTimeColumn}").isNull() &  # current row
                                    col(f"staged_updates.{toTimeColumn}").isNull() &  # still active
                                    updateCond  # historic diff
                            ),
                            set={
                                toTimeColumn: ("staged_updates." + fromTimeColumn)
                            }
                        ...
                        mergeTableMinMax \
                            .whenMatchedUpdate(
                            condition=(col("existingDF." + toTimeColumn).isNull()) & updateCond,
                            set={
                                toTimeColumn: ("staged_updates." + fromTimeColumn)
                            }
                        ).whenNotMatchedInsert(
