...
class xml(DatasetSpec):
    ...
    docUrl: str = "https://docs.prophecy.io/engineers/xml/"
    ...
    def validate(self, context: WorkflowContext, component: Component) -> list:
        ...
        # whitespace characters are not allowed in element names in XML, so we cannot allow whitespaces in schema names.
        if component.properties.schema is not None:
            for idx, fieldName in enumerate(component.properties.schema.fieldNames()):
                if re.search(r'\s', fieldName):
                    diagnostics.append(
                        Diagnostic(f"properties.schema.fields[{idx}].name",
                                   "Field name cannot contain whitespace characters",
