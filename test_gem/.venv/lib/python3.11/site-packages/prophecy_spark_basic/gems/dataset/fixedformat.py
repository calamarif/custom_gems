...
class fixedformat(DatasetSpec):
    ...
    docUrl: str = "https://docs.prophecy.io/engineers/fixed-format/"
    ...
    def onChange(self, context: WorkflowContext, oldState: Component, newState: Component) -> Component:
        ...
        try:
            ...
            if oldProps.fixedSchemaString != newProps.fixedSchemaString:
                newSchemaString = newProps.fixedSchemaString.encode().decode('raw_unicode_escape')
                schemaJson = parse(newSchemaString)
                schemaStruct = toSpark(newSchemaString)
                newState = newState.bindProperties(dataclasses.replace(newState.properties, schema=schemaStruct, fixedSchemaJson=schemaJson))
    ...
    class FixedFormatFormatCode(ComponentCode):
        ...
        def sourceApply(self, spark: SparkSession) -> DataFrame:
            ...
            if self.props.fixedSchemaString is not None:
                schema = parse(self.props.fixedSchemaString.encode().decode('raw_unicode_escape'))
