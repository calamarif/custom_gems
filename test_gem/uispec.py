...
class ExpressionBuilderType(Enum):
    ...
    COPILOT_EXPRESSION = "CopilotExpression"
...
class Checkbox(Atom):
    ...
    def withToolTip(self, tooltip: str):
        return replace(self, tooltip=tooltip)
...
class NumberBox(Atom):
    ...
    def withMax(self, max: int):
        return replace(self, maxValueVar=max)
...
@dataclass
class PipelineSelector(Atom):
    titleVar: str
    propertyVar: Optional[str] = None
    _kind: str = "Atoms.PipelineSelector"
    ...
    def bindParameterSet(self, parameterSet: str):
        return replace(self, parameterSet=parameterSet)
    
    def withHint(self, hint: str):
        return replace(self, hint=hint)

    def jsonProperties(self) -> dict:
        props = super(PipelineSelector, self).jsonProperties()
        props["title"] = self.title()
        if self.hint is not None:
            props["hint"] = self.hint
        if self.parameterSet is not None:
            props["parameterSet"] = self.parameterSet
        return props


@dataclass
class AnalysisAtom(Atom):
    titleVar: str
    propertyVar: Optional[str] = None
    _kind: str ="Atoms.AnalysisAtom"
...
@dataclass
class FileTriggerConfigurationSelector(Atom):
    titleVar: str
    connectionId: str
    propertyVar: Optional[str] = None
    _kind: str = "Atoms.FileTriggerConfigurationSelector"
    hint: Optional[str] = None
    ...
    def bindConnectionId(self, connectionId: str):
        return replace(self, connectionId=connectionId)
    
    def withHint(self, hint: str):
        return replace(self, hint=hint)

    def jsonProperties(self) -> dict:
        props = super(FileTriggerConfigurationSelector, self).jsonProperties()
        props["title"] = self.title()
        props["connectionId"] = self.connectionId
        if self.hint is not None:
            props["hint"] = self.hint
        return props

@dataclass
class PipelineParameters(Atom):
    titleVar: str
    propertyVar: Optional[str] = None
    _kind: str = "Atoms.PipelineParameters"
    pipelineId: Optional[str] = None
    selectedPset: Optional[str] = None
    hint: Optional[str] = None
    ...
    def bindPipelineId(self, pipelineId: str):
        return replace(self, pipelineId=pipelineId)
    
    def bindSelectedPset(self, selectedPset: str):
        return replace(self, selectedPset=selectedPset)
    
    def withHint(self, hint: str):
        return replace(self, hint=hint)

    def jsonProperties(self) -> dict:
        props = super(PipelineParameters, self).jsonProperties()
        props["title"] = self.title()
        if self.pipelineId is not None:
            props["pipelineId"] = self.pipelineId
        if self.hint is not None:
            props["hint"] = self.hint
        if self.selectedPset is not None:
            props["selectedPset"] = self.selectedPset
        return props
...
class SqlSecretSelector(Atom):
    ...
    ignoreTitle: bool = False
    propertyVar: Optional[str] = None
    supportedSubKinds: Optional[list[str]] = None
    hint: Optional[str] = None
    ...
    def withSupportedSubKinds(self, supportedSubKinds: list[str]):
        return replace(self, supportedSubKinds=supportedSubKinds)
    ...
    def jsonProperties(self) -> dict:
        ...
        if not self.ignoreTitle:
            props["title"] = self.title()
        props["placeholder"] = self.placeholder
        if self.supportedSubKinds is not None:
            props["supportedSubKinds"] = self.supportedSubKinds
        if self.hint is not None:
            props["hint"] = self.hint
        return props
...
class RadioOption:
    ...
    hint: Optional[str] = None
@dataclass
class DatabricksClusterSelector(Atom):
    """
    This class is used to create a DatabricksClusterSelector.
    """
    titleVar: str
    propertyVar: Optional[str] = None
    apiParameters: list[str] = field(default_factory=list)
    api: str = ""

    """
    This method is used to get the kind of the DatabricksClusterSelector.
    """
    def kind(self) -> str:
        return "Atoms.DatabricksClusterSelector"

    """
    Title is required by Atom base class, though not used in properties here.
    """
    def title(self) -> str:
        return self.titleVar

    """
    This method is used to get the property of the DatabricksClusterSelector.
    """
    def property(self) -> Optional[str]:
        return self.propertyVar

    """
    This method is used to bind the property in which cluster ID will be stored
    """
    def bindProperty(self, property: str):
        return replace(self, propertyVar=property)

    """
    This method is used to specify the parameters required to call the List Cluster API.
    """
    def bindApiParameter(self, apiParameter: str):
        apiParameters = self.apiParameters
        apiParameters.append(apiParameter)
        return replace(self, apiParameters=apiParameters)

    """
    This API will be called with all the parameters selected in bindApiParameters 
    """
    def bindApi(self, api: str):
        return replace(self, api=api)

    """
    This method is used to get the json properties of the DatabricksClusterSelector.
    """
    def jsonProperties(self) -> dict:
        properties = super(DatabricksClusterSelector, self).jsonProperties()
        api_params_obj = {}
        if self.apiParameters is not None and len(self.apiParameters) > 0:
            # Build the structured apiParameters object
            api_params_obj = {
                "kind": "${component.kind}",
                "name": "${component.name}",
                "properties": {param: self.propertyPath(param) for param in self.apiParameters}
            }
        properties["apiParameters"] = api_params_obj
        if not isBlank(self.api):
            properties["api"] = self.api
        return properties
...
class RadioGroup(Atom):
    ...
    def addOption(self, label: str, value: str, icon: Optional[str] = None, description: Optional[str] = None
                  , hint: Optional[str] = None):
        if self.options is not None:
            self.options.append(RadioOption(label, value, icon, description, hint))
            return replace(self, options=self.options)
        else:
            return replace(self, options=[RadioOption(label, value, icon, description, hint)])
        ...
        if self.optionProperty is not None:
            ...
            for opt in self.options:
                ...
                if opt.hint is not None:
                    option_data["hint"] = opt.hint

                optionsJsonArray.append(option_data)
            properties["options"] = optionsJsonArray
...
@dataclass
class ExamineSpecButton(Atom):
    titleVar: str
    propertyVar: Optional[str] = None
    filePath: Optional[str] = None
    schema: Optional[str] = None
    autoInferSchema: bool = True
    _kind: str = "Atoms.ExamineSpecButton"
    disabled: Optional[bool] = None
    ...
    def bindFilePath(self, filePath: str):
        return replace(self, filePath=filePath)
    
    def bindSchema(self, schema: str):
        return replace(self, schema=schema)
    
    def bindAutoInferSchema(self, autoInferSchema: bool):
        return replace(self, autoInferSchema=autoInferSchema)

    def withDisabled(self, disabled: bool = True):
        return replace(self, disabled=disabled)
    ...
    def jsonProperties(self) -> dict:
        props = super(ExamineSpecButton, self).jsonProperties()
        props["title"] = self.title()
        if self.filePath is not None:
            props["filePath"] = self.propertyPath(self.filePath)
        if self.disabled is not None:
            props["disabled"] = self.disabled
        if self.label is not None:
            props["label"] = self.label
        if self.schema is not None:
            props["schema"] = self.propertyPath(self.schema)
        props["autoInferSchema"] = self.autoInferSchema
        return props
...
class Text(Container[Element]):
    ...
    def withItalic(self, italic: bool = True):
    ...
    def jsonProperties(self) -> dict:
        ...
        if self.italic is not None:
            properties["italic"] = self.italic

        return properties

@dataclass
class Ellipsis(Container[Element]):
    property: Optional[str] = None
    _children: list = None
    tooltip: str = 'false'
...
class SchemaTable(Atom):
    ...
    propertyVar: Optional[str] = None
    allowedHiddenTags: Optional[List[str]] = None
    fixedFormat: Optional[bool] = None
    providerType: Optional[str] = None
    ...
    def withAllowedHiddenTags(self, tags: List[str]):
        return replace(self, allowedHiddenTags=tags)

    def withFixedFormat(self, fixedFormat: bool = True):
        return replace(self, fixedFormat=fixedFormat)

    def addHiddenTag(self, tag: str):
        if self.allowedHiddenTags is None:
            return replace(self, allowedHiddenTags=[tag])
        else:
            self.allowedHiddenTags.append(tag)
            return replace(self, allowedHiddenTags=self.allowedHiddenTags)

    def withProvider(self, provider: str):
        return replace(self, providerType=provider)
    ...
    def jsonProperties(self):
        ...
        if self.allowedHiddenTags is not None:
            properties["allowedHiddenTags"] = self.allowedHiddenTags
        if self.fixedFormat is not None:
            properties["fixedformat"] = self.fixedFormat
        if self.providerType is not None:
            properties["providerType"] = self.providerType
        return properties
...
class OAuthButton(Atom):
    ...
    appRegistrationId: Optional[str] = None
    defaultAppRegistrationId: Optional[str] = None
    ...
    def withAppRegistrationId(self, appRegistrationId: str):
        return replace(self, appRegistrationId=appRegistrationId)

    def withDefaultAppRegistrationId(self, defaultAppRegistrationId: str):
        return replace(self, defaultAppRegistrationId=defaultAppRegistrationId)

    def withDefaultProvider(self, defaultProvider: str):
        return replace(self, defaultProvider=defaultProvider)
    ...
    def jsonProperties(self) -> dict:
        ...
        if self.appRegistrationId is not None:
            props["appRegistrationId"] = self.appRegistrationId
        if self.defaultAppRegistrationId is not None:
            props["defaultAppRegistrationId"] = self.defaultAppRegistrationId
        if self.defaultProvider is not None:
            props["defaultProvider"] = self.defaultProvider
        return props

@dataclass
class RegistrationSelector(Atom):
    _title: str
    _kind: str = "Atoms.RegistrationSelector"
    propertyVar: Optional[str] = None
    provider: Optional[str] = None
    defaultProvider: Optional[str] = None
    defaultValue: Optional[str] = None
    ...
    def withDefaultValue(self, defaultValue: str):
        return replace(self, defaultValue=defaultValue)

    def withDefaultProvider(self, defaultProvider: str):
        return replace(self, defaultProvider=defaultProvider)

    def jsonProperties(self) -> dict:
        props = super(RegistrationSelector, self).jsonProperties()
        if self.provider is not None:
            props["provider"] = self.provider
        if self.defaultProvider is not None:
            props["defaultProvider"] = self.defaultProvider
        if self.defaultValue is not None:
            props["defaultValue"] = self.defaultValue
        return props
...
class TextBoxTemplate(Container[Element]):
    ...
    def create_project_template(titleVar: str, identifier: str):
        sub_template: StackLayout = StackLayout().addTemplate(
            StackLayout(align="space-between", direction="horizontal").addElement(
                child=Ellipsis(tooltip = 'true', _children = [Condition().ifEqual(PropExpr("projects.disabled"), BooleanExpr(True))
        ...
        return TextBoxTemplate(titleVar, identifier, templateVar=sub_template,
                               _children=[Ellipsis(tooltip = 'true', _children=[NativeText("${record.label}")])])
    ...
    def create_fabric_template(titleVar: str, identifier: str):
        sub_template: StackLayout = StackLayout().addTemplate(
            StackLayout(direction="horizontal").addElement(
                ...
                .addElement(child=Ellipsis(tooltip='true', _children=[Text(_children=[NativeText("${fabrics.label}")])]))))
...
class PipelineConfigurationTable(Atom):
    ...
    _selectedProjectInstanceProperty: str = "${component.properties.configurations.selectedProjectInstance}"
    _projectInstancesProperty: str = "${component.properties.configurations.projectInstances}"
    ...
    def selectedProjectInstanceProperty(self) -> Optional[str]:
        return self._selectedProjectInstanceProperty

    def projectInstancesProperty(self) -> Optional[str]:
        return self._projectInstancesProperty
    ...
    def jsonProperties(self) -> dict:
        ...
        props["selectedProjectInstance"] = self.selectedProjectInstanceProperty()
        props["projectInstances"] = self.projectInstancesProperty()
...
@dataclass
class StoredProcedureArgumentTable(Atom):
    titleVar: str
    parameters: Optional[Dict[str, str]] = None
    storedProcedureIdentifier: Optional[str] = None
    propertyVar: Optional[str] = None
    ...
    def kind(self) -> str:
        return "Atoms.StoredProcedureArgumentTable"

    def bindProperty(self, property: str):
        return replace(self, propertyVar=property)

    def bindStoredProcedureIdentifier(self, identifier: str):
        return replace(self, storedProcedureIdentifier=identifier)

    def propertyKey(self) -> str:
        return "parameters"

    def jsonProperties(self) -> dict:
        properties = super(StoredProcedureArgumentTable, self).jsonProperties()
        if self.parameters is not None:
            properties["parameters"] = self.propertyPath(self.parameters)
        if self.storedProcedureIdentifier is not None:
            properties["storedProcedureIdentifier"] = self.propertyPath(self.storedProcedureIdentifier)
        return properties


@dataclass
class StoredProcedureSelectBox(Atom):
    titleVar: str
    storedProcedureIdentifier: Optional[str] = None
    propertyVar: Optional[str] = None

    def kind(self) -> str:
        return "StoredProcedureSelectBox"
    ...
    def jsonProperties(self):
        properties = super(StoredProcedureSelectBox, self).jsonProperties()
        if self.storedProcedureIdentifier is not None:
            properties["storedProcedureIdentifier"] = self.propertyPath(self.storedProcedureIdentifier)
        return properties
...
@enum.unique
class MergeApproachType(enum.Enum):
    columns = "columns"
    scd2 = "scd2"
    ...
    deleteAndInsert = "delete_and_insert"

    @staticmethod
    def get_enum(value):
        if value == "columns":
            return MergeApproachType.columns
        elif value == "scd2":
            return MergeApproachType.scd2
        elif value == "delete+insert":
            return MergeApproachType.deleteInsert
        elif value == "delete":
            return MergeApproachType.delete
        elif value == "insert_overwrite":
            return MergeApproachType.insertOverwrite
        elif value == "replace_where":
            return MergeApproachType.replaceWhere
        elif value == "delete_and_insert":
            return MergeApproachType.deleteAndInsert
        return None
...
class WriteModeType(enum.Enum):
    ...
    def get_enum(value):
        if value == "overwrite":
            return WriteModeType.overwrite
        elif value == "append":
            return WriteModeType.append
        elif value == "merge":
            return WriteModeType.merge
        elif value == "errorIfExists":
            return WriteModeType.errorIfExists
...
@dataclass
class TargetWriteOptions(Atom):
    titleVar: str
    propertyVar: Optional[str] = None
    columns: str = "${component.ports.inputs[0].schema.fields}"
    writeOptions: str = "${component.properties.format.properties.writeOptions}"
    providerType: str =  "${component.properties.connector.kind}"
    supportedMergeApproaches: List[MergeApproachType] = field(default_factory=list)
    supportedWriteModes: List[WriteModeType] = field(default_factory=list)

    def kind(self) -> str:
        return "Atoms.TargetWriteOptions"
    ...
    def withSupportedMergeApproaches(self, approaches: List[MergeApproachType]):
        return replace(self, supportedMergeApproaches=approaches)

    def withSupportedWriteModes(self, modes: List[WriteModeType]):
        return replace(self, supportedWriteModes=modes)

    def bindProperty(self, property: str):
        return replace(self, propertyVar=property)

    def jsonProperties(self) -> dict:
        properties = super(TargetWriteOptions, self).jsonProperties()
        properties["columns"] = self.columns
        properties["writeOptions"] = self.writeOptions
        properties["providerType"] = self.providerType
        if self.supportedMergeApproaches is not None:
            properties["supportedMergeApproaches"] =  [t.value for t in self.supportedMergeApproaches]
        if self.supportedWriteModes is not None:
            properties["supportedWriteModes"] = [t.value for t in self.supportedWriteModes]
        return properties
...
class Schedule(Atom):
    titleVar: str
    cronVar: Optional[str] = None
    timeZoneVar: Optional[str] = None
    propertyVar: Optional[str] = None
    defaultCronVar: Optional[str] = None
    frequencyOptionsVar: Optional[List[str]] = None

    def title(self) -> str:
        return self.titleVar

    def kind(self) -> str:
        return "Atoms.Schedule"

    def property(self) -> Optional[str]:
        return None

    def bindProperty(self, property: str):
        return replace(self, cronVar=property)

    def bindCron(self, property: str):
        return replace(self, cronVar=property)

    def bindTimeZone(self, property: str):
        return replace(self, timeZoneVar=property)

    # Renamed method
    def withDefaultCron(self, cron_string: str):
        return replace(self, defaultCronVar=cron_string)

    def withFrequencyOptions(self, options: List[str]):
        """Sets the list of frequency options (e.g., ['Daily', 'Hourly'])."""
        return replace(self, frequencyOptionsVar=options)

    def jsonProperties(self) -> dict:
        props = super(Schedule, self).jsonProperties()

        if self.cronVar is not None:
            props["cron"] = self.propertyPath(self.cronVar)
        if self.timeZoneVar is not None:
            props["timeZone"] = self.propertyPath(self.timeZoneVar)
        if self.defaultCronVar is not None:
            props["defaultCron"] = self.defaultCronVar
        if self.frequencyOptionsVar is not None:
            props["frequencyOptions"] = self.frequencyOptionsVar

        return props

@dataclass
class KGProgressBanner(Atom):
    #empty atom just for placeholder
    def title(self) -> str:
        return ""
    ...
    def kind(self) -> str:
        return "Atoms.KGProgressBanner"

    def bindProperty(self, property: str):
        return self

    def jsonProperties(self) -> dict:
        properties = super(KGProgressBanner, self).jsonProperties()
        return properties